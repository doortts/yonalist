...
font stack. Soft Paper keeps both the general textarea and row-title offsets at
`-1px` for Avenir Next. The page title uses the general field geometry while
outline rows use the row-title override, so the values remain independently
configurable. Notes CSS consumes only the custom properties, so
