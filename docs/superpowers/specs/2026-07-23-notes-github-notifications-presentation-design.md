...
날짜 그룹과 하위 블릿의 소속은 최초 로딩 때 고정하지 않는다. 자동 폴링, 수동 새로고침, 부분 결과 수신 및 완료 후 스냅샷 갱신처럼 Notifications 소스가 바뀔 때마다 최신 `updated_at`과 그 시점의 로컬 날짜를 기준으로 필터링과 그룹화를 다시 수행한다.

- 같은 thread에 새 활동이 생겨 `updated_at`이 바뀌면 다음 소스 갱신에서 알맞은 날짜 그룹으로 이동한다.
- 날짜 경계가 바뀌면 다음 소스 갱신에서 `Today`, `Yesterday` 라벨과 하위 블릿 소속을 다시 계산한다.
- 이동이나 삭제로 자식이 없어진 날짜 블릿은 제거한다.
- 새 그룹과 자식 순서는 다시 최신순으로 정렬한다.
- 새로고침이 실패하면 마지막 정상 스냅샷의 그룹을 유지하고, 다음 정상 갱신에서 다시 계산한다.
...
투영 결과는 저장된 날짜 계층을 증분 수정하지 않고 매 소스 스냅샷에서 다시 만든다. 따라서 이전 `parentKey`를 별도로 정리하는 상태 머신 없이도 갱신된 알림이 정확한 날짜 부모를 갖고, 빈 날짜 그룹이 남지 않는다.
...
- 자동·수동·부분 결과 갱신에서 변경된 `updated_at`에 따라 기존 알림이 다른 날짜 부모로 이동한다.
- 로컬 날짜 경계 뒤 다음 소스 갱신에서 `Today`와 `Yesterday` 그룹이 다시 계산된다.
- 이동하거나 사라진 알림 때문에 비게 된 날짜 그룹은 제거된다.
- 갱신 실패에는 마지막 정상 그룹을 유지하고 다음 정상 갱신에서 재구성한다.
...
3. Notifications 소스가 갱신될 때 최신 활동 시각과 현재 날짜에 맞춰 하위 블릿의 날짜 소속과 순서가 다시 조정된다.
4. 각 알림은 Type 아이콘과 기존 Notifications 부제 형식의 한 줄 note를 표시한다.
5. 알림에는 잘못된 펼침 화살표가 없다.
6. 웹 아이콘으로 정확한 GitHub 대상 페이지가 시스템 브라우저에서 열린다.
7. 완료, 캐시, 계정 격리, 오류 처리 및 일반 Notes 데이터 경계가 회귀하지 않는다.
8. 집중 테스트, 전체 프런트엔드 테스트, lint와 production build가 통과한다.
9. 개발 빌드에서 All 목록, 날짜 그룹, 아이콘, note, 웹 열기와 완료를 직접 확인할 수 있다.
