...
- 일반 readonly와 GN 소유 row의 잠금 아이콘은 같은 action rail에서 hover 또는 `:focus-within`일 때만 보인다. 아이콘은 장식성이라 tab stop을 추가하지 않고, Type과 `ExternalLink` 버튼은 텍스트 대체 이름과 tooltip을 유지한다.
