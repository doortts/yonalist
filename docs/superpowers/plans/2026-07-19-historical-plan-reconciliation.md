...
- 고정 manifest `23/23`, checkbox `684/684`
- `complete 634`, `superseded 50`, 누락·중복·증거 오류 `0`
- 감사 전 checked `26`, 감사 후 checked `634`
- Graphite 50개는 통합 뒤 revert된 증거 때문에 미체크 유지

상세 근거는 [한국어 감사 보고서](../reports/2026-07-19-historical-plan-reconciliation.md)와
[machine-readable ledger](../reports/2026-07-19-historical-plan-ledger.json)에 있다.
...
- [x] **Step 1: manifest schema와 기준값을 기록한다**
...
- [x] **Step 5: Git과 artifact 검증을 구현한다**
...
- [x] **Step 1: 기준 commit까지의 날짜·subject·변경 경로를 수집한다**
...
- [x] **Step 2: 각 계획이 도입·수정한 경로의 history를 확인한다**
...
- [x] **Step 3: verification report와 실제 test 파일을 교차 확인한다**
...
- [x] **Step 1: 각 checkbox를 task 단위로 commit diff와 artifact에 대조한다**
...
- [x] **Step 3: 각 문서 상단에 사용자용 현재 상태를 추가한다**
...
- [x] **Step 4: disposition에 맞게 checkbox를 수정한다**

`complete`만 `[x]`, 나머지는 `[ ]`로 둔다. 문구와 task 순서는 수정하지 않는다.

- [x] **Step 5: 부분 검증을 실행한다**
...
- [x] **Step 1: 342개 ordinal을 같은 증거 규칙으로 대조한다**
...
- [x] **Step 1: multiselect와 whole-app redesign 증거를 세부 항목별로 대조한다**
...
- [x] **Step 2: 기존 checked 7+19 항목도 commit과 artifact를 재검증한다**
...
- [x] **Step 1: ledger에서 disposition 합계를 출력하는 기능을 추가한다**
...
- [x] **Step 2: 한국어 보고서에 기준과 전체 결과를 기록한다**
...
- [x] **Step 2: complete group의 commit과 artifact를 무작위가 아닌 전수 재검증한다**
...
- [x] **Step 3: 문서·validator 경계에 비례한 최종 gate만 실행한다**
...
- [x] **Step 5: 불일치 수정이 있으면 최종 교정 commit을 만든다**
...
- [x] 고정 manifest가 정확히 23개 문서와 684개 checkbox를 가리킨다.
- [x] 684개 ordinal이 ledger에서 누락·중복 없이 정확히 한 번 분류된다.
- [x] 모든 checked box는 도달 가능한 commit과 기준 commit의 artifact/test 증거가 있다.
- [x] partial/superseded/unimplemented 항목은 unchecked이고 한국어 근거가 있다.
- [x] 23개 문서의 상단 상태, checkbox, ledger, 한국어 보고서가 서로 일치한다.
- [x] 새 2026-07-19 계획 checkbox는 역사 통계에서 제외된다.
- [x] validator test, Git evidence 검사, diff 검사가 통과하고 제외한 gate 이유가 기록된다.
