...
|---|---|---|
| 어디서나 | `\` → `\\`, `&` → `&amp;`, `<` → `&lt;` | `/`, `.`, `+`, `,`, 괄호, 문장 안의 `-`, `*`, `_`, 백틱 |
| 줄 첫 글자 | 블록을 여는 접두사에만 백슬래시 — `[ ]`·`[x]`, 선두 `-`·`*`·`+`·`#`, `[`가 뒤따르는 선두 `!`, 코드 fence ```` ``` ````·`~~~`, 가로줄 `---`·`***`·`___`, 선두 숫자열(최대 9자리)을 닫는 `.`이나 `)`. 표시 앞의 공백 최대 3칸까지 허용하고 백슬래시는 **표시 앞**에 붙는다 | `Shift+Enter`, `음.`, `1.5배`, `-5도`, 문장 중간의 `a > b` |
| 링크 label | 위에 더해 `[` → `\[`, `]` → `\]` | 파일명의 `.`, `-` |

구현이 이 표에서 벗어난 곳이 셋이고 셋 다 근거가 있다.

**`>`는 escape하지 않는다.** 초안은 줄 첫 글자의 `>`를 `&gt;`로 바꾸라고 했다. 필요 없다 — note는 자기 `> `를 앞에 쓰고 reader는 `>` 하나와 공백 하나를 다시 떼므로 두 번째 `>`는 그대로 살아 돌아온다. root note, 자식 note, `>` 하나뿐인 줄, 공백 없는 `>`, note 안의 빈 줄, `>`로 시작하는 블릿 본문을 전부 확인했다. 이 포맷이 `>`를 문자 아닌 것으로 읽는 자리는 없다.

**`[ ]`·`[x]`가 표에 없었다.** 이것이 이 항목에서 유일하게 병합을 깨뜨린 구멍이었다. reader가 `- [ ] `와 `- [x] `를 줄 앞에서 떼므로 본문이 그렇게 시작하는 블릿은 todo로 돌아오고 세 글자를 잃는다. 본문이 정확히 `[ ]`이면 더 나쁘다 — 줄이 `- [ ] <!-- yid: … -->`가 되어 경계 검색이 뗄 주석을 못 찾고 노드가 자기 id를 본문 안으로 삼킨다. `content_of_file`이 `text`·`marker`·`completed` 셋을 다 해시하므로 값이 바뀌는 변경이고 §0이 금지한 그것이다.

**fence·가로줄과 선두 공백을 더했다.** 이 reader는 셋 다 문자로 되돌려 주므로 왕복은 안전하지만, 손대지 않은 ```` ``` ````는 사람이 파일을 연 편집기에서 나머지 전부를 삼킨다. A1이 "일반 편집기에서 읽고 고친다"이므로 그건 표시 문제가 아니다.
...
`parse.rs`의 `unescape_scan`은 한 글자도 고치지 않는다. 이미 `\` + ASCII 문장부호 전부를 되돌리므로 renderer가 백슬래시를 덜 쓰는 방향은 그 상위집합 안에 있다. 성립하는 성질은 `unescape(escape(v)) == v`이고 그것이 필요한 전부다 — 역방향 `escape(unescape(s)) == s`는 성립하지 않으며(`\.` → `.` → `.`) 요구되지도 않는다. 파일이 정본이 아니라 값이 정본이다.
...
**`two_devices.rs`**: 파일을 건드리지 않고 통과한다. 텍스트 값은 그대로이고 파일 인코딩만 바뀌므로 `content_of_file`의 digest도 그대로다 — 단 `[ ]`·`[x]`는 예외이고 그것이 escape 목록에 들어가야 하는 이유다(§2.6).

기존 vault에 대한 관측 가능한 결과는 **없다.** `export.rs`의 방출 여부는 노드 값의 fingerprint에 걸려 있고 그 값은 움직이지 않으므로, 이미 있는 파일은 어떤 노드가 실제로 바뀔 때까지 옛 바이트(`것만\.`)를 그대로 들고 있다. A1은 새로 쓰이는 문서에서 먼저 보이고 기존 문서에서는 편집이 닿을 때 보인다. 초안이 "첫 방출이 모든 파일을 다시 쓴다"고 적은 것은 틀렸다.
