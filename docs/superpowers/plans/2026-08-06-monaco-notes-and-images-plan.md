...
- 래스터 이미지 백엔드 재작업. v2 백엔드(서명·MIME·바이트·픽셀 검증, 내용
  주소 자산 저장, resize/replace IPC)는 이미 완성돼 있고 그대로 재사용한다.
  SVG는 이 검증 경로를 통과할 수 없으므로(벡터라 `image` 크레이트가 디코드
  하지 않음) 전용 파이프라인을 추가한다 — §2c·Phase 3S.
...
| # | 계약 | 근거 |
|---|---|---|
| I1 | PNG/JPEG/WebP/GIF/**SVG**를 픽커·클립보드·파일 드롭으로 삽입 | spec §Images + 2026-08-06 SVG 포함 결정(§2c) |
...
### 2c. SVG 지원 (레거시 제외 항목의 범위 확대)

레거시 스펙은 SVG를 "능동 콘텐츠라 별도 보안 설계 필요"를 이유로 제외했다.
2026-08-06 사용자 결정으로 이번 범위에 포함하되, **그 별도 보안 설계를 이
계획이 함께 수행한다**. 아래가 SVG를 다른 포맷과 동등하게 취급하기 위한
추가 계약이다.

| # | 계약 | 근거 |
|---|---|---|
| S1 | SVG는 임포트 시 **소독된 바이트로 변환해 저장**한다. 원본 바이트는 보존하지 않는다 | 저장 파일이 OS 핸들러로 열릴 수 있음(`notes_view_image_original`) |
| S2 | 소독은 제거 목록 기반이 아니라 **허용 목록(allowlist)** 기반이다. 허용 외 요소·속성은 전부 제거 | 우회 가능한 블랙리스트 금지 |
| S3 | `<!DOCTYPE>`·`<!ENTITY>` 선언이 있으면 **파싱 전에 거부**한다 | XML 엔티티 확장(billion laughs) DoS 차단 |
| S4 | `script`, `foreignObject`, `use`의 외부 참조, `image`의 비-data URI, `on*` 이벤트 속성, `href`/`xlink:href`의 비-data 스킴, CSS `@import`·`url()` 외부 참조를 제거 | 능동 콘텐츠·외부 유출 차단 |
| S5 | 요소 수·중첩 깊이·소독 후 바이트에 상한을 두고 초과 시 거부 | 파싱·렌더 DoS 차단 |
| S6 | 픽셀 크기는 `width`/`height`, 없으면 `viewBox`에서 유도한다. 둘 다 없으면 거부 | 스키마가 `pixel_width/height > 0`을 요구, I3 폭 규칙의 입력 |
| S7 | 렌더는 기존 경로 그대로 `<img src=blob:>` — 브라우저의 secure static mode에서 스크립트·외부 로드가 차단된다. 이를 **두 번째 방어선으로만** 취급하고 S1~S5를 생략하지 않는다 | 심층 방어 |
| S8 | PDF 내보내기는 SVG를 래스터화해 임베드하거나, 불가하면 해당 이미지를 건너뛰고 사용자에게 알린다 | PDF 렌더러가 SVG를 임베드하지 못함 |

스키마 영향: `notes_images.mime_type` CHECK 제약에 `'image/svg+xml'`을
추가해야 한다. v2는 스키마 버전 1 고정이고 마이그레이션 경로가 없으므로
(`architecture.md` "There are no migration ... tables"), **스키마 버전 2와
최소 마이그레이션 도입**이 이 계획의 선행 작업이 된다(Phase 3S).
...
- **SVG 위협 모델과 소독 명세**: 허용 요소·속성 목록 전문, 거부 사유 코드,
  상한값(요소 수·깊이·바이트), 크기 유도 규칙, 소독 전후 해시·중복 제거
  영향. 부록으로 반드시 거부돼야 하는 악성 샘플 목록(스크립트 실행,
  엔티티 폭탄, 외부 리소스 유출, 이벤트 핸들러, `use` 외부 참조)
...
### Phase 3S — SVG 파이프라인 (Opus 5 구현, Phase 3와 직렬)

보안 계층이라 게이트를 별도로 둔다. 테스트가 명세를 선행한다.

- 스키마 버전 2 + 최소 마이그레이션(`notes_images` 재작성으로 mime CHECK
  확장). 기존 DB에서 데이터 보존 확인 테스트, 버전 불일치 거부 테스트
- 소독기 단위 테스트를 **거부 샘플 부록 전수**로 작성:
  `<script>` 실행, `onload`/`onclick` 속성, `<foreignObject>` 내 HTML,
  `xlink:href="http://…"` 유출, `<use href="external.svg#x">`,
  `<!ENTITY>` 폭탄, `@import` CSS, `<image href="file://…">`,
  깊이·요소 수 초과, 크기 정보 없음
- 소독 결과의 멱등성(두 번 소독해도 동일), 소독 후 해시로 중복 제거 동작
- 크기 유도 테스트: width/height 있음 · viewBox만 있음 · 퍼센트 단위 ·
  둘 다 없음(거부)
- 통합: SVG 삽입 → 표시 → 리사이즈 → undo → 재시작 복원
- 저장된 파일이 소독된 바이트임을 확인(원본 미보존)
...
- Phase 1~5 결과물에 대해 계약 표(N·I·S) 기준 반례 탐색 리뷰: IME 조합 중
  Shift+Enter, 접힌 부모 안의 노트, 이미지 줄에서의 전체 선택 삭제,
  분할 페인 동시 편집, ingest 중 세션 충돌
- **SVG 소독 우회 전용 리뷰**: 허용 목록의 빈틈(네임스페이스 트릭, 대소문자
  변형, 유니코드 이스케이프, 중첩 인코딩, CDATA, 속성값 내 개행), 소독기와
  렌더러의 파서 해석 차이(parser differential)
...
| **SVG 소독 우회** — 계획 전체에서 가장 큰 보안 리스크 | 허용 목록 기반(S2), 원본 미보존(S1), 렌더는 secure static mode(S7)의 3중 방어. Phase 3S 거부 샘플 전수 테스트. Phase 6 적대적 리뷰에서 우회 시도를 별도 항목으로 수행 |
| 스키마 버전 2 도입이 기존 사용자 DB를 손상 | 마이그레이션은 `notes_images` 한 테이블 재작성으로 최소화. 기존 데이터 보존 테스트를 통과해야 병합. 실패 시 명확한 거부(현행 버전 불일치 동작 유지) |
| SVG가 PDF 내보내기를 깨뜨림 | S8대로 래스터화 또는 건너뛰기+안내. Phase 3S 범위 밖이면 내보내기 경로에서 SVG를 명시적으로 처리하고 이월 기록 |

## 6. 진행 규칙

- 브랜치: `feature/monaco-notes-images`, Phase별 worktree 격리 병렬화는
  파일 충돌이 없는 Phase 3(이미지 표시)과 Phase 2(노트 UI)에만 허용.
  Phase 1은 공유 기반이라 단독 선행. Phase 3S는 보안 계층이라 병렬 금지 —
  단독으로 진행하고 리뷰를 통과해야 다음으로 간다.
