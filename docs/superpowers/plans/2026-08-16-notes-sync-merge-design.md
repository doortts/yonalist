...
| 11 | 자리 주장은 파일에 실리는 (값, 스탬프) 켤레다 — `prev:` 토큰(기본값 생략), `sync_prev_hlc` 컬럼, 내용과 독립한 LWW | §7.6 |
...
## 7.5 순서 수렴 — 반례와 후보 판정 (결정은 §7.6)

**상태 수렴은 성립한다. 순서 수렴은 성립하지 않았다.** §8의 생성기가 반례를
줄여서 내놓았다.
...
원인은 하나다: **주장이 파일에 실려 있지 않다.** 파일은 줄 순서만 싣고 읽는 쪽이
줄 순서에서 주장을 파생한다. 움직이지 않은 노드의 줄 위치는 남들이 움직인 결과라서
두 파일이 같은 노드에 대해 다른 파생 주장을 내밀고, 스탬프까지 같으니 그 사이를
가를 규칙이 없다.

`sync_prev`(주장을 행에 남김), "스탬프가 나아간 노드만 주장", sequence() 재구성 —
이 셋이 **한 번이라도 움직인 노드**를 가환으로 만들었고, 남은 구멍이 위의
첫 도착 주장이었다.

후보 판정:

- **(a) 첫 도착 주장에 문서 `max_hlc`를 싣는다 — 기각.** 그대로는 수렴하지
  않는다: 두 번째 파일이 올 때는 행이 이미 있어 첫 도착 경로를 타지 않으므로
  자기 주장을 낼 기회 자체가 없고, 반례가 그대로 남는다. 수렴시키려면 "max_hlc가
  더 새 문서의 주장이 무이동 행의 주장도 덮는다"까지 가야 하는데 그것은 문서의
  줄 순서 전체를 통째로 LWW하는 규칙이 된다 — B 기기가 텍스트 하나를 고쳐 max_hlc를
  올리는 순간, A 기기가 동시에 한 이동이 fleet 전체에서 되돌아간다. 이동이
  이동 아닌 것에 진다.
- **(c) 형제 순서를 노드 스탬프로 — 기각하되 좁은 형태는 채택.** 화면 순서가
  편집 최신순이 되는 것은 아웃라이너로서 틀렸다. 다만 "주장끼리 겨룰 때의 동률
  해소"라는 좁은 형태는 이미 sequence()의 (스탬프, id) 동률 규칙으로 들어와 있고,
  §7.6이 같은 계열의 digest 동률 하나를 더한다.
- **(b) 주장을 파일에 싣는다 — 채택.** 주장이 값째 파일을 타면 처음 보는 쪽도
  파생이 아니라 **쓴 쪽이 실제로 들고 있던 주장**을 받는다. 위 반례에서 A·B 두
  파일 모두 n3의 주장을 "n2 뒤, 생성 스탬프"로 싣고 오므로 겨룰 것 자체가 없다.
  포맷 변경이지만 §9의 `format_version` 규율은 **릴리스** 규율이라(같은 빌드
  fleet에서는 발화하지 않는다) 승급이 필요 없고, 결정 15의 무마이그레이션 원칙은
  제자리 수정 + 개발 DB 재생성으로 지킨다 — `sync_prev`가 이미 지난 길이다.
  §4.3의 토큰 문법에는 한 행으로 들어가고(`:` 토큰은 다음 단어 하나를 값으로
  소비), §5.3의 미지 토큰 보존은 건드리지 않는다 — `prev:`는 인정 토큰이 된다.

`prop_merge_order_of_two_docs_does_not_change_the_state`와
`prop_two_dbs_converge_by_exchanging_exports`는 약화하지 않고 §7.6의 항목이
문안 그대로 쓴다. 멱등 property는 지금 초록이다.

## 7.6 결정 — 자리 주장은 파일에 실리는 (값, 스탬프) 켤레다

### 7.6.1 규칙

한 노드는 독립한 레코드 둘을 갖는다. **내용**은 지금처럼 t(hlc)가 지키고,
**자리**는 자기 켤레 **(prev 값, 주장 스탬프)**가 지킨다. 내용이 지면서 자리가
이길 수 있고 그 반대도 된다 — 지금은 자리 채택이 내용 판정에 얹혀 있는데, 그
결합이 "내용은 지고 이동만 새 것"인 노드의 자리를 잃게 하므로 판정을 가른다.

주장 스탬프를 노드의 t와 겸용하지 않는 이유가 이 설계의 핵심이다: 겸용하면
텍스트 편집이 행을 재스탬프하는 순간 옛 주장이 최신 스탬프로 되살아나
sequence()의 층이 뒤바뀐다 — **편집이 이동처럼 보인다.** 주장 스탬프는 자리가
실제로 바뀐 순간에만 나아간다.

1. **저장.** `notes_nodes.sync_prev`(있음) + `sync_prev_hlc`(신규 컬럼, 제자리
   수정 — 개발 DB는 §12의 절차로 재생성한다. 버전 승급 없음). 빈
   `sync_prev_hlc`는 "아직 병합이 쓴 적 없는 로컬 저작분" 표시다(규칙 5).
2. **파일.** 노드 주석에 `prev: <앞형제-uuid 또는 빈 값>@<hlc>` — `from:`처럼 한
   단어 값이고 마지막 `@`에서 가른다. 빈 값은 "첫 형제"다. **주장이 (바로 앞
   형제 줄의 id, 노드 자신의 t)와 같으면 생략한다** — 생성 순서 그대로인 문서는
   오늘과 같은 바이트로 나가고, 토큰은 파생과 실제가 갈라진 노드에만 남는다.
   토큰이 없거나 값이 파싱되지 않으면 파서가 (앞 줄의 id, t)로 채운다(§5.1의
   관용과 같은 결). 값이 가리키는 id가 파일 안에 없어도 격리하지 않는다 — 부모를
   건너 옮겨진 노드의 옛 follower가 정상적으로 그런 값을 든다.
3. **판정 — 주장 스탬프의 LWW.** 파일 주장 스탬프 > 행 주장 스탬프면 채택, 작으면
   유지하고 write-back을 표시한다. 같은데 값이 다르면 §2.3과 같은 두 갈래다:
   스탬프의 device가 내 것이면 저작(규칙 4), 남의 것이면 값 digest가 큰 쪽 —
   결정적이고 교환적이다. 첫 도착(행 없음)은 파일의 켤레를 그대로 기록한다.
4. **내 vault의 손 이동은 줄이 이긴다.** 줄에서 파생한 앞형제와 행의 주장 값이
   다른데 주장 스탬프가 내 device면, 토큰이 무엇을 싣고 있든 fresh 주장 스탬프로
   줄 순서를 채택하고 write-back한다 — §6 손편집 계약의 자리판이다. 남의
   스탬프면 토큰이 이긴다: 원격 손 이동은 §6 그대로 그 기기의 앱이 재스탬프해
   돌아온다.
5. **로컬 저작분 fallback.** `sync_prev_hlc`가 빈 행은 SiblingOrder 적재 때
   (sort_key상 바로 앞 live 형제, 행의 hlc)로 읽는다. 로컬 이동은 sort_key를
   바꾸고 트리거가 hlc를 fresh로 찍으므로 이 fallback이 이동 직후의 진짜 자리를
   최신 스탬프로 주장한다 — 지금 코드에서 로컬 이동 뒤 첫 flush가 stale
   `sync_prev`로 그 이동을 화면에서 되돌리는 구멍이 이 규칙으로 같이 닫힌다.
6. **sequence()·flush는 그대로다.** `Claim.stamp`의 출처만 hlc에서
   `sync_prev_hlc`(빈 값이면 fallback)로 바뀐다. 겨루는 주장의 동률은 지금처럼
   (스탬프, id)다.
7. **첫 도착 append는 빈 스탬프를 판다.** 위치 없는 문서 루트(§1.5-1)가 목록 끝에
   붙을 때의 주장 스탬프는 `''`다 — home의 진짜 줄이 언제 도착해도 이긴다. 지금은
   append가 노드 스탬프를 파는데, 페이지 제목 편집이 그 스탬프를 올려 두면 home의
   더 오래된 진짜 자리가 자리표시에 지는 역전이 있다.
8. **split 줄이 prev를 싣는다.** 위치 권위가 줄에 있기 때문이다(스펙 §4.5).
   trash 루트는 싣지 않는다 — 자리는 `from:`이 이미 준다. trash 자식은 싣는다.
9. **로컬 이동의 주장 유지는 M3.2 의무다.** 이동 명령이 이동 노드와 옛·새
   follower 셋의 (sync_prev, sync_prev_hlc)를 이동 스탬프로 쓴다 — 이동 하나가
   건드리는 행은 최대 셋이지 형제 전부가 아니다. 그 전까지는 fallback(규칙 5)이
   이동 노드를 지키고 follower는 "같은 스탬프·다른 값" 동률로 떨어져 digest가
   결정적으로 가른다 — 수렴은 유지되고 픽이 임의적일 뿐이다. exporter(M4)의
   `prev:` 방출도 같은 fallback을 쓴다(§12 메모).

반례 재검: A·B 모두 n3를 `prev: n2@t0`로 싣는다(생략 규칙에 따라 실제로는 A에서
n1·n3의 토큰만 나온다 — 이동 때문에 파생과 실제가 갈라진 자리다). 어느 순서로
병합해도 n3의 켤레는 (n2, t0)이고, n1의 켤레는 A의 이동이 재연결한 (n2, tA)가
B의 (빈 값, t0)을 이긴다. 두 순서 모두 [n2, n1, n3] — 이동한 기기의 화면
그대로다.

### 7.6.2 이 결정이 무효화하는 것

- **§7.5의 기존 장치 "스탬프가 나아간 노드만 자리를 주장"** — 자리 판정이 자기
  켤레의 LWW로 독립하면서 필요가 없어졌다. `write_row`의 `advanced` 기반 주장
  게이트가 이 규칙의 코드 자리다.
- **§1.5-2 "위치 증거는 적용 시점 순서로 판정한다"** — 행의 자리 증거가 sequence
  파생값에서 저장된 주장 값으로 바뀌어 적용 순서 민감성 자체가 사라진다. 메모리
  갱신은 flush 키 계산용으로만 남는다.
- **§1.5.1-1의 근거** — id 발급 위치는 그대로 두되, "뒤 형제가 손으로 친 줄을
  따른다"는 파생 주장이 사라져 그 실패 모드(손 줄 하나가 문서를 재배열)는 더
  일어나지 않는다.
- **§2.1의 정규화 내용 필드 목록에서 `parent_id`·`predecessor_id`** — 자리는
  내용 밖이다. 구현은 이미 그렇게 갔고(`place_of_file`의 주석이 근거) 이 결정이
  문서를 구현에 맞춘다. 위치 증거 문장 "파일에서는 바로 앞 형제 줄의 id"는
  "prev 토큰의 값, 없으면 앞 줄"로 읽는다.

### 7.6.3 항목 — M3.1f

| 항목 | 내용 | 합격 조건 (acceptance) | 첫 red 테스트 |
|---|---|---|---|
| **M3.1f** 자리 켤레와 대수 | `sync_prev_hlc` 컬럼, `prev:` 토큰(파스·렌더·생략 규칙), 자리 판정 독립 LWW + digest 동률 + 저작 갈래, fallback 적재, 빈 스탬프 append, 생성기의 주장 모델링(이동이 셋의 켤레를 쓴다), property 2종 | 무이동 노드의 자리가 병합 순서와 무관하게 같고, 교환·수렴 property 2종이 §8 문안 그대로 green이며, 생성 순서 그대로인 문서의 렌더 바이트가 불변이다 | `two_documents_sharing_a_base_agree_on_an_unmoved_nodes_place` (반례 모양 그대로의 단위 테스트) |

- 커밋 제목: `feat(sync): carry each node's place claim in the file and lock the merge algebra`
- 러너: `cargo test -p notes-sync`
- 바뀌는 파일: `crates/notes-sqlite/src/schema.sql`(컬럼), `crates/notes-sync/src/document.rs`(DocumentNode에 주장 켤레), `crates/notes-sync/src/parse.rs`·`render.rs`(+ golden — 생략 규칙 덕에 기존 fixture 바이트는 그대로여야 한다), `crates/notes-sync/src/merger.rs`(판정 이원화·적재·기록), `crates/notes-sync/tests/merge_algebra.rs`(생성기 확장 + property 2종 + 단위, 덤프에 sync_prev·sync_prev_hlc 추가), 스펙 §4.3·§4.5·§4.6·§11(이 결정과 같은 커밋에서 완료).
- `MergeOutcome.applied`는 자리만 채택한 쓰기도 센다 — 행이 바뀌었고 revision이 올라야 watcher 계약이 성립한다.
- 수렴 property의 render 입력은 행에서 DocumentNode를 짓는 테스트 쪽 builder가 만든다 — `prev:` 방출은 규칙 9의 fallback 포함 렌더 규칙 그대로다.

### 7.6.4 규칙 ↔ red 테스트 대응 (삭제 검증)

| 규칙 | 지우면 red가 되는 테스트 |
|---|---|
| 첫 도착이 파일의 켤레를 그대로 기록 | `two_documents_sharing_a_base_agree_on_an_unmoved_nodes_place` |
| 자리 LWW가 내용 판정과 독립 | `a_place_claim_wins_independently_of_a_content_defeat` (신규 — 내용은 지고 이동만 새 노드) |
...
### 7.6.5 스펙 개정 요지 (이 결정과 같은 커밋에서 반영 완료)

- **§4.3** — 토큰 표에 `prev`가 3번으로 들어간다(값 문법, 생략 조건, 관용 규칙
  포함). `format_version`은 1 그대로다.
- **§4.5** — split 줄도 §4.3의 조건대로 `prev`를 싣는다는 문장 하나.
- **§4.6** — trash 루트 줄은 `prev`를 싣지 않고(자리는 `from:`) 자식 줄은
  싣는다는 문장 하나.
- **§11** — golden 4종은 전부 생성 순서 그대로라 `prev`가 모두 생략된다 —
  바이트 불변이라는 각주 하나.
...
- M3.2 — 로컬 이동 명령이 이동 노드와 옛·새 follower의 (sync_prev, sync_prev_hlc)를 이동 스탬프로 쓴다(§7.6 규칙 9). 그 전까지의 공백은 fallback과 digest 동률이 결정적으로 메운다.
- M3.4 — loser_json `v: 1` 스키마(§7)가 읽기·복구의 wire 계약이다. `reason`을 목록에 노출한다.
- M4.1 — exporter는 `hlc = ''` 행(자리표시)을 방출에서 건너뛴다(§3·§6). 정규화 내용 함수(§2.1)를 `sync_node_exports.content_hash`에 재사용한다. `prev:` 방출은 §7.6 규칙 2의 생략 규칙과 규칙 5·9의 fallback을 그대로 쓴다.
