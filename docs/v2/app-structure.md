...
2026-08-05, Monaco 아웃라인이 기본 표면이 된 시점의 구조 문서다.
...
- 아웃라인 표면 선택은 `outlineSurface.ts` 한 곳에서 한다. 기본은 Monaco,
  `?outline=react`로만 React 표면을 강제한다. 단 페이지에 노트(설명)나
  이미지 노드가 있으면 Monaco 세션이 텍스트 전용이라는 제약 때문에
  자동으로 React 표면으로 내려간다. 이 폴백이 살아 있는 동안 React
  아웃라인 계열 코드는 삭제하면 안 된다.
...
2. **메타데이터** — `OutlineLineMetadata[]`가 줄 번호와 1:1로 대응하며
   nodeId·parentId·depth·collapsed·completed를 담는다.
...
- Monaco 세션은 텍스트 전용 불릿만 지원한다. 노트(설명)·이미지·Todo
  마커·태그/날짜 하이라이트는 React 폴백에만 있다. Monaco에서 지원하려면
  모델-메타데이터 매핑 확장 설계가 필요하다.
- 휴지통/복제/행 이동 단축키와 불릿 드래그는 `IpcEditorCommand`에
  해당 커맨드(deleteSubtree, duplicate 계열)를 추가한 뒤 붙인다.
- React 아웃라인 폴백 제거는 위 두 항목에 더해 **partial-viewport 조건**까지
  풀어야 가능하다: Monaco 로드는 50,000노드 단일 뷰포트 쿼리를 전제하므로
  (`storeMonaco.ts`), 그보다 큰 페이지는 지금도 React 표면으로 내려간다.
  점진 로딩을 설계하거나 50k를 제품 상한으로 못박아야 한다.
