...
export function SidebarPaneIcon({ size = 16, ...props }: PaneIconProps) {
  return (
    <svg
      ...
      aria-hidden="true"
      data-pane-icon="sidebar-collapse"
      {...props}
    >
...
/**
 * Reopen glyph for a hidden left sidebar. It keeps the same rounded outer pane
 * shape, but right-aligns the divider so the collapsed control reads as
 * opening the pane back from the window edge rather than closing it again.
 */
export function SidebarPaneOpenIcon({ size = 16, ...props }: PaneIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
