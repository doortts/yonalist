...
interface OutboxModalProps {
  ...
  onEdit?: (operation: OutboxOperationDocument) => void;
  onDelete?: (operation: OutboxOperationDocument) => void;
  onSync: () => void;
  onClose: () => void;
}
                  ...
                  return (
                    <article className="outbox-card" key={operation.frontMatter.id}>
                      ...
                      <button
                        ...
                        type="button"
                        aria-label={`Open target for ${operationName}`}
                        onClick={() => onOpenTarget?.(operation)}
                        disabled={!onOpenTarget}
                      >
                        <strong>{operationName.replace("_", " ")}</strong>
                      ...
                      <div className="outbox-card-actions">
                        {onEdit && (
                          <button
                            className="icon-button outbox-action-button"
                            type="button"
                            aria-label={`Edit ${operationName}`}
                        ...
                        {onDelete && (
                          <button
                            className="icon-button outbox-action-button outbox-delete-button"
                            type="button"
                            aria-label={`Delete ${operationName}`}
