...
describe("LruCache", () => {
  ...
  describe("onEvict callback", () => {
    it("invokes onEvict only for capacity evictions", () => {
      ...
      cache.set("a", 1);
      cache.set("b", 2);
      cache.set("c", 3); // overflows maxSize, evicts "a"

      expect(evicted).toEqual([["a", 1]]);

      // Explicit delete, overwrite, and clear must NOT fire onEvict.
      cache.delete("b");
      cache.set("c", 30); // overwrite in place, no eviction
      cache.clear();
