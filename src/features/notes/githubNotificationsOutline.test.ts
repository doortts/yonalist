...
describe("projectGithubNotificationsOutline", () => {
  it("keeps stored order, sorts dates, and appends projected rows after the saved block", () => {
    const root = node(GITHUB_NOTIFICATIONS_ROOT_ID, null, 2, {
      title: "Github Notifications",
  ...
  it("keeps disappeared stored snapshots and filters read/completed subtrees without using viewedAt", () => {
    ...
    const projection = projectGithubNotificationsOutline({
      workspace: workspace([root, date, unread, read, readChild, completeUser]),
      page: snapshot,
      showCompleted: false,
      now: new Date("2026-07-22T12:00:00Z"),
    });
  ...
  it("relabels a stored-only date group when the shared projection clock crosses midnight", () => {
    ...
    expect(
      beforeMidnight.rows.find((row) => row.kind === "date"),
    ).toMatchObject({
      kind: "date",
      title: "Today",
    });
...
describe("GitHub notification row adapters", () => {
  it("falls back to the previous visible title, then the next title", () => {
    ...
    expect(resolveGithubEditorFocusFallback(previous, [], saved)).toBeNull();
    expect(
      resolveGithubEditorFocusFallback(
    ...
    expect(
      resolveGithubEditorFocusFallback([projected], [replacement], projected),
    ).toEqual(replacement);
  });
