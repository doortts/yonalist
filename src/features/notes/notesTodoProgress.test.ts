...
import {
  buildTodoProgressMap,
  directTodoProgress,
} from "./notesTodoProgress";
...
describe("buildTodoProgressMap", () => {
  it("builds progress for every parent in one traversal result", () => {
    const nodes = [
    ...
    expect(progress.get("a")).toEqual({ completed: 1, total: 2 });
    expect(progress.has("b")).toBe(false);
    expect(progress.get("c")).toEqual({ completed: 0, total: 1 });
  });

  it("matches the direct lookup semantics for every parent", () => {
    const nodes = [
      node("a-done", "a", "todo", true),
      node("a-open", "a", "todo"),
    ...
    const progress = buildTodoProgressMap(nodesById, childIdsByParent);

    for (const parentId of Object.keys(childIdsByParent)) {
      expect(progress.get(parentId) ?? null).toEqual(
        directTodoProgress(parentId, nodesById, childIdsByParent),
      );
    }
  });

  it("reflects immutable node changes when rebuilt", () => {
