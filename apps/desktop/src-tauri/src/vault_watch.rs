...
fn run(
    ...
    loop {
        ...
        while let Some(relative) = queue.next_in_flight(now()) {
            // Asked between files, not only between rounds. A round can be a
            // whole folder — the boot scan is — and a user quitting the app
...
const PLACEHOLDER_NODE: &str = "00000000-0000-4000-8000-AcnSSU4wyaI3";
...
mod tests {
    ...
    fn documents_and_the_attachments_beside_them_are_watched() {
        assert_eq!(
            seen("Projects-26VJSt4Rw5eO/README.md").as_deref(),
            Some("Projects-26VJSt4Rw5eO/README.md")
        );
        assert_eq!(
            seen("Projects-26VJSt4Rw5eO/assets/shot-9f3a1c8e2044.png").as_deref(),
            Some("Projects-26VJSt4Rw5eO/assets/shot-9f3a1c8e2044.png")
        );
    ...
    fn images_outside_an_assets_folder_are_left_alone() {
        ...
        assert_eq!(seen("Projects-26VJSt4Rw5eO/notes.txt"), None);
    }
    ...
    fn a_watch_lets_go_of_its_folder_when_it_is_dropped() {
        ...
        // Generous on purpose: what is being asserted is that it ends at all.
        // The suite runs a dozen of these threads at once, so a deadline tight
        // enough to measure promptness would only measure the machine.
        assert!(
            waiting.recv_timeout(Duration::from_secs(60)).is_ok(),
    ...
    const WAITING_NODE: &str = "TueFvJq1xQdA";
    ...
    fn waiting_picture() -> notes_sync::document::VaultFile {
        ...
        VaultFile::Page(PageDocument {
            id: DocumentId::Node("26VJSt4Rw5eO".to_owned()),
            parent: None,
            sort_key: None,
            max_hlc: hlc.clone(),
